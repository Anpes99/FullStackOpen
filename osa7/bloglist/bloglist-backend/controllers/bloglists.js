...
bloglistsRouter.post('/:id/comment', async (req, response) => {
  ...
  const commentedBlog = await Blog.find({})
    .then((blogs) => blogs.find((blog) => blog.id === req.params.id));
  commentedBlog.comments.push(comment);
  console.log('commenting blog')
